...
class Hook_points_banner__click
{
    ...
    {
        if (!addon_installed('banner_click_points') || !addon_installed('points')) {
            return null;
        }

        require_code('points');
        $data = points_ledger_calculate(LEDGER_TYPE_RECEIVED | LEDGER_TYPE_SENT | LEDGER_TYPE_SPENT, $member_id_of, null, ' AND t_type=\'banner\' AND t_subtype=\'click\'');
