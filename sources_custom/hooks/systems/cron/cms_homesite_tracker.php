...
class Hook_cron_cms_homesite_tracker
{
    ...
    public function run(?int $last_run)
    {
        ...
        $rows = $GLOBALS['SITE_DB']->query_parameterised('SELECT * FROM mantis_bug_history_table WHERE ' . db_string_equal_to('field_name', 'sponsorship_total') . ' AND date_modified>={last_run}', ['last_run' => $last_run]);
        foreach ($rows as $row) {
        ...
        $rows = $GLOBALS['SITE_DB']->query_parameterised('SELECT * FROM mantis_bug_history_table WHERE ' . db_string_equal_to('field_name', 'status') . ' AND date_modified>={last_run}', ['last_run' => $last_run]);
        foreach ($rows as $row) {
            $old_value = intval($row['old_value']);
            $new_value = intval($row['new_value']);
