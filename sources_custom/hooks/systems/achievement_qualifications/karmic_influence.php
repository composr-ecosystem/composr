...
/*
    ...
    1) count    -- A member must have at least this much karmic influence for the qualification to be satisfied (not specified: 3)
*/

/**
 * Hook class.
 */
class Hook_achievement_qualifications_karmic_influence
{
    /**
    ...
    public function run(int $member_id, array $params, ?int $last_time = null) : ?array
    {
        ...
        // Get karmic influence
        require_code('karma');
        $count_done = get_karmic_influence($member_id);

        return [intval(floor($count_done)), $count_required];
    }
    ...
    public function to_text(int $member_id, array $params, int $count_done, int $count_required) : ?object
    {
        ...
        // Finalise
        return do_lang_tempcode('ACHIEVEMENT_KARMIC_INFLUENCE_REQUIREMENT', protect_from_escaping($progress));
    }
}
