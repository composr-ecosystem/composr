...
/*
    ...
    1) count            -- The number of GitLab comments a member must make before this qualification is satisfied (not specified: 100)
    4) days             -- Only consider comments made within this many days (not specified: no filter)
*/

/**
 * Hook class.
 */
class Hook_achievement_qualifications_gitlab_comments
{
    /**
    ...
    public function run(int $member_id, array $params, ?int $last_time = null) : ?array
    {
        ...
        $where = ' WHERE receiving_member={member_id} AND status={ledger_normal} AND ' . db_string_equal_to('t_type', 'gitlab') . ' AND ' . db_string_equal_to('t_subtype', 'comment');
    ...
    public function to_text(int $member_id, array $params, int $count_done, int $count_required) : ?object
    {
        ...
        if ($days !== null) {
            $conditions->attach(do_lang_tempcode('ACHIEVEMENT_GITLAB_COMMENTS_REQUIREMENT_DAYS', escape_html(integer_format($days))));
        }
        ...
        // Finalise
        return do_lang_tempcode('ACHIEVEMENT_GITLAB_COMMENTS_REQUIREMENT', protect_from_escaping($conditions), protect_from_escaping($progress));
    }
}
