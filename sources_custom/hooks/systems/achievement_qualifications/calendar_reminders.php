...
/*
    ...
    1) count        -- The number of calendar reminders a member must subscribe to for this qualification to be met (not specified: 3)
*/

/**
 * Hook class.
 */
class Hook_achievement_qualifications_calendar_reminders
{
    /**
    ...
    public function info(int $member_id, array $params) : ?array
    {
        ...
        return [
            'supports_persist' => false, // Date and time is not tracked for subscriptions
    ...
    public function run(int $member_id, array $params, ?int $last_time = null) : ?array
    {
        ...
        // Build query
        $count_done = $GLOBALS['SITE_DB']->query_select_value('calendar_reminders', 'COUNT(*)', ['n_member_id' => $member_id]);
    ...
    public function to_text(int $member_id, array $params, int $count_done, int $count_required) : ?object
    {
        ...
        // Finalise
        return do_lang_tempcode('ACHIEVEMENT_CALENDAR_REMINDERS_REQUIREMENT', protect_from_escaping($progress));
    }
}
