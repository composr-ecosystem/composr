...
class Hook_search_cns_own_pt extends FieldsSearchHook
{
    ...
    public function index_for_search($since = null, &$total_singular_ngram_tokens = null, &$statistics_map = null)
    {
        ...
        $db = $GLOBALS['FORUM_DB'];
        $sql = 'SELECT p.*,t_cache_first_post_id,t_pt_from,t_pt_to FROM ' . $db->get_table_prefix() . 'f_posts p JOIN ' . $db->get_table_prefix() . 'f_topics t ON p.p_topic_id=t.id';
        ...
        foreach ($rows as $row) {
            ...
            foreach (array('t_pt_from', 't_pt_to') as $for_field) {
    ...
    public function run($content, $only_search_meta, $direction, $max, $start, $only_titles, $content_where, $author, $author_id, $cutoff, $sort, $limit_to, $boolean_operator, $where_clause, $search_under, $boolean_search)
    {
        ...
        if ((cron_installed()) && (get_value('composr_fulltext_indexing__cns_own_pt', '1', true) == '1') && ((intval(get_value('fulltext_max_ngram_size', '2', true)) <= 1) || (strpos($content, '"') === false))) {
