...
class Module_galleries
{
    ...
    public function uninstall()
    {
        ...
        require_code('files');
        if (!$GLOBALS['DEV_MODE']) {
            deldir_contents(get_file_base(true) . '/uploads/galleries', true);
            deldir_contents(get_file_base(true) . '/uploads/galleries_thumbs', true);
        }
    }
    ...
    public function install(?int $upgrade_from = null, ?int $upgrade_from_hack = null)
    {
        ...
        if (($upgrade_from !== null) && ($upgrade_from < 11)) {
            ...
            do {
                ...
                foreach ($rows as $row) {
                    @unlink(get_file_base(true) . '/' . urldecode($row['thumb_url']));
                }
                $start += $max;
            } while (!empty($rows));
    ...
    public function pre_run() : ?object
    {
        ...
        if ($type == 'image' || $type == 'video') {
            ...
            if ($type == 'image') {
                $image_url = baseify($myrow['url']);
            } else {
                $image_url = baseify($myrow['thumb_url']);
            }

            $closed_captions_url = '';
            if ($type == 'video') {
                $closed_captions_url = baseify($myrow['closed_captions_url']);
            }
    ...
    public function do_gallery_carousel_mode(object $rating_details, object $comment_details, string $cat, string $root, object $description, bool $may_download, object $edit_url, object $add_gallery_url, object $submit_image_url, object $submit_video_url, object $title, string $rep_image, int $start, int $max, string $fullname, object $sorting, array $gallery_row) : object
    {
        ...
        switch ($probe_type) {
            ...
            case 'image':
                ...
                if (url_is_local($image_url)) {
                    $file_size = file_exists(get_file_base() . '/' . rawurldecode($image_url)) ? strval(filesize(get_file_base() . '/' . rawurldecode($image_url))) : '';
        ...
        // Rep-image
        $rep_image_url = baseify($rep_image);
    ...
    public function show_nav(string $category_name, string $extra_where, string $join, int $current_id, ?int $first_id, ?int $back_id, ?int $next_id, string $root, int $x, int $n, string $current_type, string $first_type, string $back_type, string $next_type, int $wide_high, int $start, int $max, ?string $cat, string $url_sort, string $dir, string $image_select, string $video_select, ?int $days) : object
    {
        if ($back_id !== null) {
            ...
            if ($back_type === 'image') {
                ...
                $back_image_url = baseify($prev_row['thumb_url']);
                if ($back_image_url == '') {
        ...
        if ($next_id !== null) {
            ...
            if ($next_type === 'image') {
                ...
                $next_image_url = baseify($next_row['thumb_url']);
                if ($next_image_url == '') {
