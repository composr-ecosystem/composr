...
<div data-tpl="formScreenInputMapPosition" data-tpl-params="{+START,PARAMS_JSON,LATITUDE,LONGITUDE,NAME,google_apis_api_key}{_*}{+END}">
	...
	<button class="btn btn-primary btn-sm buttons--search js-click-geolocate-user-for-map-field" data-click-pd type="button">{+START,INCLUDE,ICON}NAME=buttons/search{+END} <span>{!FIND_ME}</span></button>
</div>
