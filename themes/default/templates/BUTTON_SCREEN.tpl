...
{$,NAME is required if URL is not passed}
{+START,IF_NON_PASSED,URL}
	<button class="btn btn-primary btn-scr {$REPLACE,_,-,{$REPLACE,/,--,{IMG}}}{+START,IF_PASSED,JS_BTN} {JS_BTN*}{+END}" name="{NAME*}" id="{NAME*}" rel="{REL*}"><span>{+START,IF_PASSED,IMG}{+START,INCLUDE,ICON}NAME={IMG}{+END}{+END} {TITLE*}</span></button>
{+END}
