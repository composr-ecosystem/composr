	...
	{+START,SET,CAPTCHA}
		<div class="captcha">
			{+START,IF,{$CONFIG_OPTION,audio_captcha,1}}
				<a id="captcha-audio" rel="nofollow" class="js-click-play-self-audio-link" title="{!captcha:PLAY_AUDIO_VERSION}" href="{$BASEIFY*,uploads/captcha/{$SESSION*}.wav}?cache_break={$RAND&*}">{!captcha:PLAY_AUDIO_VERSION}</a>
			{+END}
