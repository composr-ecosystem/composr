<div class="gallery-mode-screen gallery-grid-mode-screen" data-tpl="galleryGridModeScreen" itemscope="itemscope" itemtype="http://schema.org/ImageGallery">
	...
	{$SET,children,{$BLOCK,block=main_multi_content,param=gallery,render_mode=boxes,pinned=,select={CAT}>,zone={$ZONE},sort={$CONFIG_OPTION,galleries_sort_order},max={$CONFIG_OPTION,subgallery_link_limit},no_links=1,pagination=1,give_context=0,include_breadcrumbs=0,render_if_empty=0,guid=module}}
	{$SET,entries,{$TRIM,{$BLOCK,block=main_gallery_embed,param={CAT_SELECT},zone={$ZONE},days={DAYS},max={$CONFIG_OPTION,gallery_entries_grid_per_page},show_sorting=1,pagination=1,select={IMAGE_SELECT},video_select={VIDEO_SELECT},filter={FILTER},video_filter={FILTER},block_id=module,render_if_empty=1}}}
