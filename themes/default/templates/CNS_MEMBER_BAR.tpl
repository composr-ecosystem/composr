				...
				<div style="min-height: {$MAX,100,{MAX_AVATAR_HEIGHT|}}px" class="cns-member-column cns-member-column-b">
					...
					<dl class="meta-details-list">
						{+START,IF,{$ADDON_INSTALLED,points}}
							<dt class="field-name"><abbr title="{!LIFETIME_POINTS,{$INTEGER_FORMAT*,{$AVAILABLE_POINTS},0}}">{!POINTS}</abbr>:</dt> <dd><a {+START,IF_PASSED,NUM_POINTS_ADVANCE} title="{!GROUP_ADVANCE,{NUM_POINTS_ADVANCE*}}"{+END} href="{$PAGE_LINK*,site:points:member:{$MEMBER}}">{NUM_POINTS*}</a></dd>
						{+END}
