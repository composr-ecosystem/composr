...
foreach ($files as $i => $file) {
    ...
    $IN = cms_file_get_contents_safe(get_file_base() . '/' . $file, FILE_READ_LOCK);
    ...
    if ($IN != $out) {
        if (get_param_integer('debug', 0) == 1) {
            ...
            cms_file_put_contents_safe(get_file_base() . '/' . $file, $out, FILE_WRITE_FIX_PERMISSIONS);
        }
    }
}
echo 'Finished!';
...
if ($limit_file == '') {
    cms_file_put_contents_safe(get_file_base() . '/data/guids.bin', serialize($GUID_LANDSCAPE), FILE_WRITE_FIX_PERMISSIONS);
}
