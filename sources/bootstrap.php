...
/*
    ...
    This file should be kept to an absolute minimum because it is the only core file which cannot run under strict types.
...
function require_code__bootstrap(string $codename)
{
    ...
    if ($GLOBALS['DEV_MODE']) {
        $code = clean_php_file_for_eval__bootstrap(file_get_contents(get_file_base__bootstrap() . '/sources/' . $codename . '.php'));
        ...
        try {
            ...
            $errortrace = $e->getTraceAsString(); // Dev mode sites should never run in production, so provide a trace
        } catch (Error $e) {
            $result = false;
        ...
        $errortrace = str_replace($FILE_BASE, '', $errortrace);

        if ($errormsg != '') {
            exit('<!DOCTYPE html>' . "\n" . '<html lang="EN"><head><title>Critical startup error</title></head><body><h1>Composr startup error</h1><p>An error occurred trying to bootstrap the software: ' . $errormsg . "\n" . $errortrace . '</p><p>The core developers maintain full documentation for all procedures and tools, especially those for installation. These may be found on the <a href="https://composr.app">Composr website</a>. If you are unable to easily solve this problem, we may be contacted from our website and can help resolve it for you.</p><hr /><p style="font-size: 0.8em">Composr is a website engine created by Christopher Graham.</p></body></html>');
        }
    } elseif (!is_file(get_file_base__bootstrap() . '/sources/' . $codename . '.php')) {
        ...
        require_once(get_file_base__bootstrap() . '/sources/' . $codename . '.php');
    }
}
