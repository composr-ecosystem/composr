...
function messages_script()
{
    ...
    if ($action == 'all') {
        ...
        require_code('files');
        cms_file_put_contents_safe(get_file_base(true) . '/data_custom/modules/chat/chat_last_event.bin', strval($event_id), FILE_WRITE_FIX_PERMISSIONS);
        ...
        foreach ($pruned as $p) {
            ...
            if ($p['room_id'] === null) {
                ...
                if (($last_become_inactive === null) || ($last_become_active > $last_become_inactive)) { // If not already marked inactive
                    ...
                    require_code('files');
                    $path = get_file_base(true) . '/data_custom/modules/chat';
                    cms_file_put_contents_safe(get_file_base(true) . '/data_custom/modules/chat/chat_last_event.bin', strval($event_id), FILE_WRITE_FIX_PERMISSIONS);
                }
            } else {
                ...
                if (!is_guest($p['member_id'])) {
                    ...
                    if ($left_room_msg != '') {
                        ...
                        require_code('files');
                        cms_file_put_contents_safe(get_file_base(true) . '/data_custom/modules/chat/chat_last_msg.bin', strval($message_id), FILE_WRITE_FIX_PERMISSIONS);
                    }
                }
            }
        }
        ...
        foreach ($hooks as $hook => $ob) {
            if (method_exists($ob, 'reply_to_any_communication')) {
                ...
                if ($response !== null) {
                    ...
                    require_code('files');
                    cms_file_put_contents_safe(get_file_base(true) . '/data_custom/modules/chat/chat_last_msg.bin', strval($bot_message_id), FILE_WRITE_FIX_PERMISSIONS);
                }
            }
        }
...
function chat_get_room_content(?int $room_id, array $_rooms, ?int $max_messages = null, bool $dereference = false, bool $downloading = false, ?int $start = null, ?int $finish = null, ?int $uptoid = null, ?string $zone = null, ?int $entering_room = null, bool $return_my_messages = true, bool $return_system_messages = true) : array
{
    ...
    if (($downloading) || ($uptoid !== null) || (!$return_my_messages)) {
        ...
        $rows = $GLOBALS['SITE_DB']->query($query, $max_messages, 0, false, false, array_key_exists('chat_messages', $TABLE_LANG_FIELDS_CACHE) ? $TABLE_LANG_FIELDS_CACHE['chat_messages'] : []);
    } else {
