...
class Forum_driver_cns extends Forum_driver_base
{
     ...
     */
    public function find_emoticons(?int $member = null) : array
    {
        require_code('cns_forum_driver_helper');
     ...
     */
    public function member_home_url(int $id, bool $tempcode_okay = false)
    {
     ...
     */
    protected function _member_pm_url(int $id, bool $tempcode_okay = false)
    {
     ...
     */
    protected function _forum_url(int $id, bool $tempcode_okay = false)
    {
     ...
     */
    public function topic_url(int $id, string $forum = '', bool $tempcode_okay = false)
    {
        if ($id === null) {
     ...
     */
    public function cns_flood_control(int $id)
    {
        global $FLOOD_CONTROL_ONCE;
        ...
        if ((get_value('disable_flood_control') !== '1') && (running_script('index'))) {
            ...
            if (($wait_time > 0) && (addon_installed('stats'))) {
                ...
                if (($count >= $count_threshold) && (addon_installed('securitylogging'))) {
                    ...
                    dispatch_notification('auto_ban', null, do_lang('AUTO_BAN_SUBJECT', $ip, null, null, get_site_default_lang()), do_notification_lang('AUTO_BAN_DOS_MESSAGE', $ip, integer_format($count_threshold, 0), integer_format($time_threshold), get_site_default_lang()), null, A_FROM_SYSTEM_PRIVILEGED);
