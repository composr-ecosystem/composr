...
class Hook_import_cms_merge
{
    ...
    public function import_images_and_galleries(object $db, string $table_prefix, string $file_base)
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($test === null) {
                add_gallery($row['name'], $this->get_lang_string($db, $row['fullname']), $this->get_lang_string($db, $row['the_description']), $row['notes'], $row['parent_id'], $row['accept_images'], $row['accept_videos'], $row['is_member_synched'], $row['layout_mode'], $row['rep_image'], $row['watermark_top_left'], $row['watermark_top_right'], $row['watermark_bottom_left'], $row['watermark_bottom_right'], $row['allow_rating'], $row['allow_comments'], false, $row['add_date'], null, '', '', false, $row['gallery_sort'], $row['media_sort']);
            }
        }
