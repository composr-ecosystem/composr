...
class Hook_import_cms_merge
{
        ...
        foreach ($rows as $row) {
            ...
            foreach (array_keys($langs) as $lang) {
                ...
                if (is_file($file_path_from)) {
                    ...
                    cms_file_put_contents_safe($file_path_to, $comcode_new, FILE_WRITE_FIX_PERMISSIONS | FILE_WRITE_BOM);
                }
            }
    ...
    public function import_cns_polls_and_votes(object $db, string $table_prefix, string $file_base)
    {
        ...
        foreach ($rows as $row) {
            ...
            $rows3 = $db->query_select('f_poll_answers', ['*'], ['pa_poll_id' => $row['id']], 'ORDER BY id');
            $answers = [];
            $id_ordinal_map = [];
            ...
            foreach ($rows2 as $row2) {
                ...
                $GLOBALS['FORUM_DB']->query_insert('f_poll_votes', ['pv_poll_id' => $id_new, 'pv_member_id' => $row2['pv_member_id'], 'pv_answer_id' => $answer, 'pv_ip' => $row2['pv_ip']]);
            }
