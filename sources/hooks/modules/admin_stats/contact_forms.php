 ...
 * Hook class.
 */
class Hook_admin_stats_contact_forms extends CMSStatsProvider
{
    /**
    ...
    public function info($for_kpi = false)
    {
        ...
        return [
            'contact_form_submissions' => [
                'label' => do_lang_tempcode('CONTACT_FORM_SUBMISSIONS'),
                'category' => 'conversions',
                'filters' => [
                    'contact_form_submissions__month_range' => new CMSStatsDateMonthRangeFilter('contact_form_submissions__month_range', do_lang_tempcode('DATE_RANGE'), null, $for_kpi),
                    'contact_form_submissions__country' => new CMSStatsListFilter('contact_form_submissions__country', do_lang_tempcode('VISITOR_COUNTRY'), find_countries()),
                ],
                'pivot' => new CMSStatsDatePivot('contact_form_submissions__pivot', $this->get_date_pivots(!$for_kpi)),
    ...
    public function preprocess_raw_data($start_time, $end_time, &$data_buckets)
    {
        ...
        $query = 'SELECT * FROM ' . get_table_prefix() . 'stats_contact_forms WHERE ';
        ...
        do {
            ...
            foreach ($rows as $row) {
                ...
                foreach (array_keys($date_pivots) as $pivot) {
                    ...
                    if (!isset($data_buckets['contact_form_submissions'][$month][$pivot][$pivot_value][$country_code])) {
                        $data_buckets['contact_form_submissions'][$month][$pivot][$pivot_value][$country_code] = 0;
                    }
                    $data_buckets['contact_form_submissions'][$month][$pivot][$pivot_value][$country_code]++;
                }
            }
