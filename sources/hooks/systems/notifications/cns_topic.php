...
class Hook_notification_cns_topic extends Hook_Notification
{
    ...
    protected function _is_member($only_if_enabled_on__notification_code, $only_if_enabled_on__category, $member_id)
    {
        ...
        if (is_numeric($only_if_enabled_on__category)) { // Also merge in people monitoring forum
            $forum_details = $GLOBALS['FORUM_DB']->query_select('f_topics', ['t_forum_id', 't_pt_from', 't_pt_to'], ['id' => intval($only_if_enabled_on__category)]);
    ...
    public function list_members_who_have_enabled($notification_code, $category = null, $to_member_ids = null, $from_member_id = null, $start = 0, $max = 300)
    {
        ...
        if (is_numeric($category)) { // This is a topic. Also merge in people monitoring forum
            $forum_details = $GLOBALS['FORUM_DB']->query_select('f_topics', ['t_forum_id', 't_pt_from', 't_pt_to'], ['id' => intval($category)]);
