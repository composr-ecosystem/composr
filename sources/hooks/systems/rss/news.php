...
class Hook_rss_news
{
    ...
    public function run(string $_filters, int $cutoff, string $prefix, string $date_string, int $max) : ?array
    {
        ...
        $query = 'SELECT DISTINCT p.* FROM ' . $GLOBALS['SITE_DB']->get_table_prefix() . 'news p LEFT JOIN ' . get_table_prefix() . 'news_category_entries d ON d.news_entry=p.id' . $extra_join . ' WHERE date_and_time>' . strval($cutoff) . (((!has_privilege(get_member(), 'see_nonvalidated')) && (addon_installed('validation'))) ? ' AND validated=1 ' : '') . ' AND ' . $filters . $extra_where  . ' ORDER BY date_and_time DESC';
