    ...
    switch ($tag) {
        ...
        case 'url':
            ...
            if (array_key_exists('rel', $attributes)) {
                $rel = array_flip(preg_split('#\s+#', trim(strtolower($attributes['rel']))));
            } else {
                $rel = array();
            }
            ...
            $temp_tpl->attach(do_template('COMCODE_URL', array('_GUID' => 'd1657530e6d3d57e6a4791fb3bfa0dd7', 'TITLE' => $title, 'REL' => implode(' ', array_keys($rel)), 'TARGET' => $attributes['target'], 'URL' => $url_full, 'CAPTION' => $caption)));
            break;
