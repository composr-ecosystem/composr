...
function get_tax_using_tax_codes(&$item_details, $field_name_prefix = '', $shipping_cost = 0.00, $member_id = null)
{
    ...
    foreach ($non_taxcloud_item_details as $i => $parts) {
        ...
        if ($tax_code == 'EU') {
            require_code('http');
            list($__rates) = cache_and_carry('cms_http_request', ['http://euvat.ga/rates.json', ['convert_to_internal_encoding' => true, 'timeout' => 20.0]]);
...
function form_input_tax_code($set_title, $description, $set_name, $default, $required, $tabindex = null)
{
    ...
    require_code('http');
    list($__tics) = cache_and_carry('cms_http_request', ['https://prev.taxcloud.net/tic/?format=json', ['convert_to_internal_encoding' => true, 'timeout' => 20.0, 'trigger_error' => false]]);
