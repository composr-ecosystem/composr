...
class TicketsEmailIntegration extends EmailIntegration
{
    ...
    {
        CMSLoggers::mail_integration()->inform('Starting overall incoming e-mail scan process (support tickets)');
        ...
        CMSLoggers::mail_integration()->inform('Finished overall incoming e-mail scan process (support tickets)');
    }
    ...
    protected function _process_incoming_message(string $from_email, string $email_bounce_to, string $from_name, string $subject, ?string $_body_text, ?string $_body_html, array $attachments)
    {
        ...
        for ($i = 0; $i < $num_matches; $i++) {
            ...
            CMSLoggers::mail_integration()->inform('Detected tag ' . $tag);
        ...
        if ($member_id === null) {
            ...
            CMSLoggers::mail_integration()->inform('Bound to member #' . strval($member_id));
        }
        ...
        CMSLoggers::mail_integration()->inform('Recording ' . $from_email . ' as a valid posted for member #' . strval($member_id));
        ...
        if ($existing_ticket_id === null) {
            ...
            CMSLoggers::mail_integration()->inform('Created new ticket, ' . $new_ticket_id);
        } else {
            // Reply to the ticket...
            ...
            CMSLoggers::mail_integration()->inform('Posted in ticket, ' . $existing_ticket_id);
        }
