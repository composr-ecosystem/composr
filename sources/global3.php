 ...
 */
function ip_banned($ip, $force_db = false, $handle_uncertainties = false, &$is_unbannable = null, &$ban_until = null, $check_caching = true)
{
    ...
    static $cache = array();
    $sz = serialize(array($force_db, $handle_uncertainties));
    ...
    if ((!$force_db) && (((isset($SITE_INFO['known_suexec'])) && ($SITE_INFO['known_suexec'] == '1')) || (is_writable_wrap(get_file_base() . '/.htaccess')))) {
        ...
        for ($i = 0; $i < $ban_count; $i++) {
            $ip_bans[$bans[2][$i]] = array('ip' => ip_apache_to_wild($bans[2][$i]));
        }
    }
...
function cms_gethostbyaddr($ip_address)
{
    ...
    if ((php_function_allowed('shell_exec')) && (function_exists('get_value')) && (get_value('slow_php_dns') === '1')) {
        $shell = shell_exec('host ' . escapeshellarg_wrap($ip_address));
        if (is_string($shell)) {
            $hostname = trim(preg_replace('#^.* #', '', $shell));
        }
    }
