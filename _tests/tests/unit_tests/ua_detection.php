...
class ua_detection_test_set extends cms_test_case
{
    public function testBotDetection()
    {
        ...
        $data = http_get_contents($url->evaluate(), array('convert_to_internal_encoding' => true, 'ignore_http_status' => true, 'trigger_error' => false, 'ua' => 'bingbot', 'cookies' => array(get_session_cookie() => get_session_id())));
        $this->assertTrue($data !== null && strpos($data, 'findpost') === false, 'Could not find findpost in ' . $url->evaluate());

        $data = http_get_contents($url->evaluate(), array('convert_to_internal_encoding' => true, 'ignore_http_status' => true, 'trigger_error' => false, 'cookies' => array(get_session_cookie() => get_session_id())));
        $this->assertTrue($data !== null && strpos($data, 'findpost') !== false, 'Could not find findpost in ' . $url->evaluate());
    }
