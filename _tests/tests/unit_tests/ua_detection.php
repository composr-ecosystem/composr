...
class ua_detection_test_set extends cms_test_case
{
    public function testBotDetection()
    {
        ...
        $data = http_get_contents($url->evaluate(), array('ignore_http_status' => true, 'trigger_error' => false, 'ua' => 'bingbot', 'cookies' => array(get_session_cookie() => get_session_id()))); // TODO #3467
        $this->assertTrue($data !== null && strpos($data, 'findpost') === false, 'Could not find findpost in ' . $url->evaluate());

        $data = http_get_contents($url->evaluate(), array('ignore_http_status' => true, 'trigger_error' => false, 'cookies' => array(get_session_cookie() => get_session_id()))); // TODO #3467
        $this->assertTrue($data !== null && strpos($data, 'findpost') !== false, 'Could not find findpost in ' . $url->evaluate());
    }
    ...
    public function testMobileDetection()
    {
        ...
        foreach ($uas as $ua) {
            $data = http_get_contents($url->evaluate(), array('ua' => $ua)); // TODO #3467
