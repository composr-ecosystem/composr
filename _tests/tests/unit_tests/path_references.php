            ...
            foreach ($regexps as $regexp) {
                ...
                for ($i = 0; $i < $num_matches; $i++) {
                    ...
                    if (!file_exists(get_file_base() . '/' . $path)) {
                        ...
                        $this->assertTrue(!file_exists(get_file_base() . '/' . cms_strtolower_ascii($path)) && !file_exists(get_file_base() . '/' . cms_strtoupper_ascii($path)), $path . ' has case sensitivity issues');
                    }
                }
            }
        }
