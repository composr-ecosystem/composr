...
class special_links_test_set extends cms_test_case
{
    ...
    {
        $c = http_get_contents('https://www.bookfinder.com/search/?isbn=0241968984&mode=isbn&st=sr&ac=qr', array('trigger_error' => false, 'timeout' => 10.0, 'ua' => false)); // seems to dislike bots // TODO #3467
        ...
        $c = http_get_contents('https://www.bookfinder.com/search/?isbn=978-0241968987&mode=isbn&st=sr&ac=qr', array('trigger_error' => false, 'timeout' => 10.0, 'ua' => false)); // seems to dislike bots // TODO #3467
        $this->assertTrue(strpos($c, 'No Place to Hide') !== false, 'External link not working, fix test and use within Composr (separate)');
    }

    public function testLookupLinks()
    {
        $this->assertTrue(strpos(http_get_contents('http://whatismyipaddress.com/ip/12.34.56.78', array('trigger_error' => false)), 'AT&T Services') !== false, 'External link not working, fix test and use within Composr (separate) [LOOKUP_SCREEN.tpl, COMMANDR_WHOIS.tpl]'); // TODO #3467
        $this->assertTrue(strpos(http_get_contents('https://ping.eu/ping/?host=12.34.56.78', array('trigger_error' => false)), 'Ping') !== false, 'External link not working, fix test and use within Composr (separate) [LOOKUP_SCREEN.tpl, COMMANDR_WHOIS.tpl]'); // TODO #3467
        $this->assertTrue(strpos(http_get_contents('https://ping.eu/traceroute/?host=12.34.56.78', array('trigger_error' => false)), 'Traceroute') !== false, 'External link not working, fix test and use within Composr (separate) [LOOKUP_SCREEN.tpl, COMMANDR_WHOIS.tpl]'); // TODO #3467
    }

    public function testWhoIsLink()
    {
        $this->assertTrue(stripos(http_get_contents('http://whois.domaintools.com/compo.sr', array('trigger_error' => false)), 'whois') !== false, 'External link not working, fix test and use within Composr (separate) [WARN_SPAM_URLS.tpl]'); // TODO #3467
    }
    ...
    public function testHealthCheckLinks()
    {
        ...
        foreach ($urls as $url => $test_no_redirecting) {
            if ($test_no_redirecting) {
                $result = cms_http_request($url, array('trigger_error' => false)); // TODO #3467
